...
# 디스크 분리
디스크는 128GB SSD와 3TB HDD를 두었다. 예상하겠지만, 128GB SSD에는 OS를 설치할 것이고 3TB HDD에는 데이터를 저장할 것이다. 이렇게 디스크를 나눈다면 시스템에 치명적인 오류가 발생하여도 HDD에 있는 데이터는 안전하게 보존할 수 있다. HDD에 저장되는 데이터를 더욱 안전하게 보관하기 위해 데이터 백업 및 복원이 가능한 btrfs 파일시스템으로 만들 것이다.
